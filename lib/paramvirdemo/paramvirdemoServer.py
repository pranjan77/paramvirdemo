...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'paramvirdemo'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from paramvirdemo.paramvirdemoImpl import paramvirdemo
impl_paramvirdemo = paramvirdemo(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['paramvirdemo.count_contigs_async'] = ['paramvirdemo', 'count_contigs']
async_check_methods['paramvirdemo.count_contigs_check'] = ['paramvirdemo', 'count_contigs']
sync_methods['paramvirdemo.count_contigs'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'paramvirdemo'
        ...
        self.rpc_service.add(impl_paramvirdemo.count_contigs,
                             name='paramvirdemo.count_contigs',
                             types=[basestring, basestring])
        self.method_authentication['paramvirdemo.count_contigs'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "paramvirdemo but no authentication header was passed"
                            raise err

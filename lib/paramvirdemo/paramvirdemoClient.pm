package paramvirdemo::paramvirdemoClient;
...
paramvirdemo::paramvirdemoClient
...
sub new
{
    ...
    my $self = {
	client => paramvirdemo::paramvirdemoClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$workspace_name is a paramvirdemo.workspace_name
$contigset_id is a paramvirdemo.contigset_id
$return is a paramvirdemo.CountContigsResults
...
# Authentication: required
	...
	method => "paramvirdemo.count_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for paramvirdemo::paramvirdemoClient\n";
    }
    if ($sMajor == 0) {
        warn "paramvirdemo::paramvirdemoClient version is $svr_version. API subject to change.\n";
    }
}
...
package paramvirdemo::paramvirdemoClient::RpcClient;
